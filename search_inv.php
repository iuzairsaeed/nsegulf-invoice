...
$sel1="select * from cards where (cards.clientjobnumber like '%".$searchname."%' OR cards.cardnumber like '%".$searchname."%') and cards.cardstatus='inv';";
$s1=mysql_query($sel1);
...
while($s1f=mysql_fetch_assoc($s1)){
	echo '<td>'.$s1f['cardnumber'].'</td><td>'.$s1f['clientjobnumber'].'</td><td>'.$s1f['carddate'].'</td><td><a href="addin2.php?cardid='.$s1f['ID'].'"/>Add Invoice</td>';
	echo '</tr>';
}
echo '</table>';
echo '</div>';
...
<form method="POST" action="search_inv.php">
	<table style="width:450px;margin:0 auto;text-align:center;margin:0 auto;">
	<tr style="font-weight:bold">
	<td><div align="left">Enter Number</div></td><td><div align="left">
