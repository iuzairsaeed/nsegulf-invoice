...
$delnum=$s1f['cardnumber'].'/D';
		$i1="insert into delivery (delnum, cardid, deldate) 
		values('".addslashes($delnum)."', '".addslashes($_POST['cardid'])."','".addslashes($cdate)."');";
		$i1res=mysql_query($i1)or die(mysql_error());
			

		$i3="update cards set cardstatus='fin' where ID='".$_POST['cardid']."';";
		$i3r=mysql_query($i3)or die(mysql_error());
		$i3="select sum(linetotal) as totalamount from services where cardid='".$_POST['cardid']."';";
		$i3res=mysql_query($i3);
		$i3f=mysql_fetch_assoc($i3res);
...
<div style="width:100%;height:40px;margin:0 auto;text-align:center;"><h1>DELIVERY NOTE</h1></div>
<div style="width:100%;height:20px;float:right;text-align:right;"><h4>Delivery #: '.$delnum.'</h4></div>
<div style="width:100%;height:20px;float:right;text-align:right;"><h4>Delivery Date: '.datareformat($_POST['cdate']).'</h4></div>
...
<table width="100%" style="vertical-align: bottom; font-family:sans;font-size: .7em;color: #555; text-align:center;border:1px solid #555;"><tr>
    <td width="99%">NSE Electronics LLC - Delivery Number:'.$delnum.'</td>
...
<td style="width:5%;border-left:1px solid #000;border-top:1px solid #000;border-bottom:1px solid #000;"><strong>Qty</strong></td>
<td style="width:95%;border-top:1px solid #000;border-bottom:1px solid #000;border-right:1px solid #000"><strong>Description</strong></td></tr></thead>';
...
<div style="width:100%;margin:0 auto;float:left;height:20px"></div>
<div style="width:100%;margin:0 auto;float:left;">
<table style="width:100%;"><tr>
<td>Received by:</td><td></td><td>Delivered By:</td><td></td></tr>
<tr><td style="height:20px;"></td><td style="height:20px;"></td><td style="height:20px;"></td><td style="height:20px;"></td></tr>
...
//$mpdf->Output($path.$month."-".$year.'.pdf','F');
	...
	$c1="select * from cards where ID = '".addslashes($_POST['cardid'])."' and cardstatus='fin';";
	$c1res=mysql_query($c1)or die(mysql_error());
	$c1r=mysql_num_rows($c1res);
