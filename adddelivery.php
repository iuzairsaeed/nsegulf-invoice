...
<?PHP
$sel1="select * from cards where cardstatus='invdone';";
$s1=mysql_query($sel1);
while($s1f=mysql_fetch_assoc($s1)){

	echo '<tr><td>'.$s1f['cardnumber'].'</td><td>'.$s1f['carddate'].'</td><td><a href="adddelivery2.php?cardid='.$s1f['ID'].'"/>Add Delivery</td>';
	echo '</tr>';
}
echo '</table>';
echo '</div>';
