...
if($_POST['submit']){
	...
	require("db.php");
	$c1="select * from cards where ID = '".addslashes($_POST['cardid'])."' and cardstatus='del';";
...
$delnum=$s1f['cardnumber'].'/D';
		...
		$i3="update cards set cardstatus='inv', totamount='".$sumf['totalline']."' where ID='".$_POST['cardid']."';";
...
<div style="width:100%;height:40px;margin:0 auto;text-align:center;"><h1>NSE DELIVERY CARD</h1></div>
...
while($delf=mysql_fetch_assoc($delr)){
	...
	$html .='<tr><td>'.$delf['qty'].'</td><td>'.$delf['description'].'</td><td>'.$delf['unitprice'].'</td><td style="text-align:right;">'.$delf['linetotal'].'</td></tr>';
...
<tr><td style="height:100px;"></td><td style="height:40px;"></td><td style="height:40px;"></td><td style="height:40px;"></td></tr>
</table>
</div>
...
<tr><td style="height:30px;"></td><td style="height:40px;"></td><td style="height:40px;"></td><td style="height:40px;"></td></tr>
