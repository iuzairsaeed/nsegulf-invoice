...
?>
	...
	<form method=POST action=addin3.php>
	<div style="width:750px;height:300px;padding-top:20px;padding-left:30px;background:#fff;">
	<table style="width:600;float:left;;text-align:left;">
	<tr>
<?PHP
$select="select * from cards where ID='".$_GET['cardid']."' and cardstatus='inv';";
$selres=mysql_query($select);
$selrow=mysql_num_rows($selres);
if($selrow != 1){
	echo "There is a problem with this invoice";
}else{
$self=mysql_fetch_assoc($selres);
}
?>
	<td></td><td><input type=hidden READONLY name=cardid value='<?php echo $_GET['cardid'] ?>'>Assigning Invoice	 Number for Card #<?PHP ECHO $self['cardnumber'] ?></td></tr><tr>
	<td style="padding-right:10px;">Date</td><td><input type=text name=cdate onclick="scwShow(this,event);"></td></tr></table><br /><br />
	
<?php
echo "<table style='width:400px;'><tr><td style='width:10px;'></td><td>Qty</td><td>Description</td><td>Unit Price</td><td>Line Total</td></tr>";
for($i=1; $i<=5; $i++){
	echo "<tr><td style='width:10px;'><input type=hidden name=number[] value=".$i."></td><td><input type=text name=quantity[]></td><td><input type=text name=description[]></td><td><input type=text name=unitprice[]></td><td><input type=text name=linetotal[]></td></tr>";
