...
$sel1="select * from cards where (clientjobnumber LIKE '%".$namesearch."%' OR cardnumber LIKE '%".$namesearch."%') and cardstatus='invdone';";
$s1=mysql_query($sel1);
...
echo '<td>Card Number</td><td>Client Job Number</td><td>Card Date</td><td>Add Delivery</td></tr>';
while($s1f=mysql_fetch_assoc($s1)){
	echo '<tr><td>'.$s1f['cardnumber'].'</td><td>'.$s1f['clientjobnumber'].'</td><td>'.$s1f['carddate'].'</td><td><a href="delin2.php?cardid='.$s1f['ID'].'"/>Add Delivery</td>';
	echo '</tr>';
}
echo '</table>';
echo '</div>';
...
?>
<div style="width:500px;padding-top:10px;background:#FFF;margin:0 auto;font:12px Trebuchet MS, sans serif;">
<form method=post action=search_del.php>
	<table style="width:450px;margin:0 auto;text-align:center;margin:0 auto;">
	<tr style="font-weight:bold">
	<td>Enter Number</td><td><input type=text name=searchname></td></tr>
	<td></td><td><input type=submit name=submit value=Search></td></tr></table></form>
</div>
